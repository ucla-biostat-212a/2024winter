...
- Once the coefficients have been estimated, we can compute the probability of `default` for any given credit card balance. For example, we predict that the `default` probability for an individual with a balance of $X=\$1,000$ is
$$
\hat p(X) = \frac{e^{\hat \beta_0+\hat\beta_1 X}}{1+e^{\hat \beta_0+\hat\beta_1 X}} =\frac{e^{-10.65 + 0.005 \times 1000}}{1 + e^{-10.65 + 0.005 \times 1000}} = 0.0058.
$$
