...
Credit: This note is compiled using content from multiple sources, e.g., [Introduction to Probability, by Dr. Dennis Sun](https://dlsun.github.io/probability/), [git repo](https://github.com/dlsun/probability). [Probability : An Introduction](https://search.library.ucla.edu/permalink/01UCS_LAL/192ecse/cdi_askewsholts_vlebooks_9780191019937) by
Geoffrey Grimmett, Dominic Welsh, and Professor of Mathematics (Retired) Dominic Welsh
...
\begin{align*}
\mathbf{E}(\mathbf{E}(Y|X)) & = \sum_{x} \mathbf{E}(Y|X = x) P(X = x)\\
& = \sum_{x} \sum_{y} y p_{Y|X} (y|x) P(X = x)\\
& = \sum_{x} \sum_{y} y p_{X,Y} (x, y)\\
& = \sum_{y} y\sum_{x} p_{X,Y} (x, y)\\
& = \sum_{y} y p_{Y} (y)\\
& = \mathbf{E}(Y)
\end{align*}
